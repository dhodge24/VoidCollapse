...
the first experimental white field. This is to capture and remove high frequency variations in our data and produce
better flat-field corrected images.
...
# Files to import
# tiffs_wfs_preprocessed = "run" + run_wfs + "_exp_preprocessed.tiff"  # If the data had no Talbot grating to begin with
tiffs_wfs_preprocessed = "run" + run_wfs + "_exp_preprocessed_no_Talbot.tiff"  # If Talbot was removed
...
wfs = np.array(imread(dir_main + dir_wfs_preprocessed + tiffs_wfs_preprocessed))

# Create directory -- if it already exists, skip. Also, clear the directory of all images.
dfx_loc = dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_dfs_x  # Directory for displacement fields
if not os.path.exists(dfx_loc):
  os.makedirs(dfx_loc)
if os.path.exists(dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_dfs_x):
    clear_directory(dir_main=dir_main + dir_wfs_to_wf1_img_reg_warped, dir_sub=dir_wfs_dfs_x)

# Create directory -- if it already exists, skip. Also, clear the directory of all images.
dfy_loc = dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_dfs_y  # Directory for displacement fields
if not os.path.exists(dfy_loc):
  os.mkdir(dfy_loc)
if os.path.exists(dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_dfs_y):
    clear_directory(dir_main=dir_main + dir_wfs_to_wf1_img_reg_warped, dir_sub=dir_wfs_dfs_y)

# Create directory -- if it already exists, skip. Also, clear the directory of all images.
warped_wf_loc = dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_warped  # Directory for warped white fields
if not os.path.exists(warped_wf_loc):
  os.mkdir(warped_wf_loc)
if os.path.exists(dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_warped):
    clear_directory(dir_main=dir_main + dir_wfs_to_wf1_img_reg_warped, dir_sub=dir_wfs_warped)

# Create directory -- if it already exists, skip. Also, clear the directory of all images.
ffc_loc = dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_ffc  # Directory for flat-field corrected images
if not os.path.exists(ffc_loc):
  os.mkdir(ffc_loc)
if os.path.exists(dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_ffc):
    clear_directory(dir_main=dir_main + dir_wfs_to_wf1_img_reg_warped, dir_sub=dir_wfs_ffc)

# Create directory -- if it already exists, skip. Also, clear the directory of all images.
jac_loc = dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_jac  # Directory for Jacobian images
if not os.path.exists(jac_loc):
  os.mkdir(jac_loc)
if os.path.exists(dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_jac):
    clear_directory(dir_main=dir_main + dir_wfs_to_wf1_img_reg_warped, dir_sub=dir_wfs_jac)

# Create directory -- if it already exists, skip. Also, clear the directory of all images.
gridlines_loc = dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_gridlines
if not os.path.exists(gridlines_loc):
  os.mkdir(gridlines_loc)
if os.path.exists(dir_main + dir_wfs_to_wf1_img_reg_warped + dir_wfs_gridlines):
    clear_directory(dir_main=dir_main + dir_wfs_to_wf1_img_reg_warped, dir_sub=dir_wfs_gridlines)

# Assign fixed and moving images
fixed = wfs[0, ...]  # Make sure that this image has decent signal-to-noise or choose another image
moving = wfs
...
# Set initial transform (Need to seed the transform with something)
...
R.SetShrinkFactorsPerLevel([256, 128, 64, 32, 16, 8, 4, 1])
R.SetSmoothingSigmasPerLevel([128, 64, 32, 16, 8, 4, 2, 1])
...
for i in tqdm(range(len(wfs))):
    ...
    # Negative Jacobian values mean that the registration is non-diffeomorphic (intensities crossing) and not valid
    if save_jac:
