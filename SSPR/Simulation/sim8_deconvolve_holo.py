...
The purpose of the code is to deconvolve our simulated/experimental images given an exact or estimated point
spread function (PSF). The PSF can be estimated by least squares error between a simulated and experimental image.
...
from SSPR.utilities import cropToCenter, showImg, FFT, IFFT, compare_plots_lineout, voigt_2d, fadeoutImage
...
class DeconvTV:
    ...
    def D(x, beta):
        ...
        horizontal (x) and vertical (y) directions, as well as across sequential frames (z-direction).
    ...
    def deconv_TV_L2(self):
        ...
        #HT_g = convolve2d(self.g, self.H, mode='same', boundary='wrap')
        HT_g = np.real(IFFT(FFT(self.g) * FFT(self.H)))
        ...
        for i in range(self.max_iter):
            ...
            if self.verbose:
                #r1 = convolve2d(self.f, self.H, mode='same', boundary='wrap') - self.g
                r1 = np.real(IFFT(FFT(self.f) * FFT(self.H))) - self.g
            ...
            rnorm_old = rnorm
            rnorm = np.sqrt(np.linalg.norm(Dfx - self.ux, 'fro') ** 2 +
                            np.linalg.norm(Dfy - self.uy, 'fro') ** 2)
    ...
    def deconv_TV_L1(self):
        """
        Deconvolution using the L1 method.
        """
        pass
...
g = np.array(imread(dir_main + dir_sim + tiff_holo_with_speckle_ffc_extended))
...
sigma_g = 535.410e-9  # Gaussian sigma
...
deconv = DeconvTV(g, H, mu=200, beta=[0.8, 0.8], rho_r=2000, gamma=2, max_iter=100, tol=1e-3, verbose=True)
...
imwrite(dir_main + dir_sim + tiff_holo_with_speckle_ffc_extended_decon, result)
