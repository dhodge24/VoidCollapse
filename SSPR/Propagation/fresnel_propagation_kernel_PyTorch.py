...
import torch
from SSPR.utilities_PyTorch import FFT
...
def fresnel_propagation_kernel_fourier(Ny, Nx, fresnel_number):
    ...
    The regime in which this propagator is valid depends on this condition: Δx >= z*lam / L --> Δx >= z*lam / (N*Δx),
    ...
    condition = 1.0/(Nx*fresnel_number)
    if condition <= 1.0:
        ...
        KX = 2 * torch.pi * torch.fft.fftshift(torch.fft.fftfreq(Nx, d=1))  # Spacing in Fourier space
        KY = 2 * torch.pi * torch.fft.fftshift(torch.fft.fftfreq(Ny, d=1))  # Spacing in Fourier space
        kx, ky = torch.meshgrid(KX, KY, indexing='ij')
        kernel = torch.exp((-1j / (4 * torch.pi * fresnel_number)) * (kx**2 + ky**2))
    else:
...
def fresnel_propagation_kernel_chirp(Ny, Nx, fresnel_number):
    ...
    if condition >= 1.0:
        ...
        # better computationally and saves time? For clarity, I will do the first option.)
        # fftfreq is the same as: 2*np.pi*np.fft.ifftshift(np.arange(np.ceil(-Nx/2), np.ceil(Ny/2), 1))/(N*dx)
        X = 2 * torch.pi * torch.fft.fftshift(torch.fft.fftfreq(Nx, d=2 * torch.pi / Nx))  # Spacing in real space
        Y = 2 * torch.pi * torch.fft.fftshift(torch.fft.fftfreq(Ny, d=2 * torch.pi / Ny))  # Spacing in real space
        x, y = torch.meshgrid(X, Y, indexing='ij')
        pre_factor = -1j * torch.abs(fresnel_number) * torch.sign(fresnel_number)
        kernel = pre_factor * torch.exp((1j * torch.pi * fresnel_number) * (x ** 2 + y ** 2))
        kernel = FFT(kernel)
    else:
        raise ValueError("Not oversampled enough for this propagator. Use another method.")
    return kernel
...
def fresnel_propagation_kernel_chirpLimited(Ny, Nx, fresnel_number):
    ...
    obliquity_factor = torch.cos((torch.pi / 2) * torch.minimum(theta_div_theta_max, torch.tensor(1)))
    kernel *= obliquity_factor
    kernel = FFT(kernel)
    return kernel
